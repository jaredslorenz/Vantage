...
import type { Insight, Investigation } from "@/types/project";
...
export function InsightPanel({
  projectId, refreshKey, investigation, investigating, onDismiss,
}: {
  projectId: string;
  refreshKey: number;
  ...
  if (investigating) {
    return (
      <div className="mb-6 bg-white/95 border border-red-200 rounded-card p-4 shadow-card flex items-center gap-3">
        <svg className="w-4 h-4 animate-spin text-red-500 shrink-0" fill="none" viewBox="0 0 24 24">
          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
          <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z" />
        </svg>
        <span className="text-[13px] text-gray-500">Investigating failure…</span>
      </div>
    );
  }
  ...
  if (investigation) {
    return (
      <div className="mb-6 bg-white/95 border border-red-200 rounded-card shadow-card overflow-hidden">
        ...
        <div className="px-5 py-4 space-y-3">
          <div className="flex items-start gap-2.5 bg-red-50 border border-red-200 rounded-lg px-3 py-2.5">
            ...
            <div>
              <span className="text-[10px] font-bold text-red-500 uppercase tracking-wider block mb-0.5">Error</span>
          ...
          <div className="flex items-start gap-2.5 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2.5">
            ...
            <div>
              <span className="text-[10px] font-bold text-amber-600 uppercase tracking-wider block mb-0.5">Root Cause</span>
              <span className="text-[12px] text-gray-700">{investigation.root_cause}</span>
            </div>
          </div>
          <div className="flex items-start gap-2.5 bg-brand-purple/5 border border-brand-purple/20 rounded-lg px-3 py-2.5">
          ...
          {investigation.key_logs.length > 0 && (
            <div className="bg-gray-950 rounded-lg p-3 space-y-0.5 max-h-36 overflow-y-auto">
  ...
  // Generating spinner (no prior insight yet)
